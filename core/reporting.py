...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .bar-row .bar-label {{ width: 260px; font-size: 13px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #e6edf3; flex-shrink: 0; }}
  .bar-row .bar-track {{ flex: 1; height: 22px; background: #0d1117; border-radius: 4px; overflow: hidden; position: relative; }}
  .bar-row .bar-fill {{ height: 100%; border-radius: 4px; transition: width 0.8s ease; }}
  ...
  .bar-row .bar-duration {{ width: 60px; text-align: right; font-size: 12px; color: #8b949e; flex-shrink: 0; }}

  .table-section {{
    background: #161b22; border: 1px solid #30363d; border-radius: 10px;
    overflow: hidden; margin-bottom: 28px;
  }}
  .table-section h2 {{ font-size: 16px; color: #8b949e; padding: 16px 20px; border-bottom: 1px solid #30363d; }}
  table {{ width: 100%; border-collapse: collapse; font-size: 14px; }}
  thead th {{ background: #0d1117; color: #8b949e; font-weight: 600; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; padding: 12px 16px; text-align: left; border-bottom: 1px solid #30363d; }}
  tbody td {{ padding: 12px 16px; border-bottom: 1px solid #21262d; vertical-align: middle; }}
  tbody tr:hover td {{ background: #1c2128; }}
  tbody tr:last-child td {{ border-bottom: none; }}
  .status-badge {{ display: inline-flex; align-items: center; gap: 6px; padding: 2px 10px; border-radius: 12px; font-size: 12px; font-weight: 600; }}
  ...
  .test-details .detail-content {{ padding: 16px 20px; font-family: 'Consolas', 'Courier New', monospace; font-size: 13px; line-height: 1.5; color: #8b949e; border-top: 1px solid #21262d; }}
  .test-details .detail-content pre {{ white-space: pre-wrap; word-break: break-word; margin: 0; }}
  .detail-toggle {{ cursor: pointer; user-select: none; opacity: 0.6; }}
  .detail-toggle:hover {{ opacity: 1; }}

  .env-section {{ background: #161b22; border: 1px solid #30363d; border-radius: 10px; padding: 20px; margin-bottom: 28px; }}
  ...
  .env-item {{ display: flex; justify-content: space-between; padding: 8px 12px; background: #0d1117; border-radius: 6px; font-size: 13px; }}
  ...
  @media (max-width: 768px) {{ .cards {{ grid-template-columns: repeat(2, 1fr); }} .chart-section {{ flex-direction: column; }} .bar-row .bar-label {{ width: 140px; }} }}
</style>
</head>
<body>
<div class="container">
  ...
  <div class="cards">
    <div class="card card-total"><div class="value">{total}</div><div class="label">Total Tests</div></div>
    <div class="card card-passed"><div class="value">{passed}</div><div class="label">Passed</div></div>
    <div class="card card-failed"><div class="value">{failed}</div><div class="label">Failed</div></div>
    <div class="card card-skipped"><div class="value">{skipped}</div><div class="label">Skipped</div></div>
    <div class="card card-rate"><div class="value">{pass_rate}%</div><div class="label">Pass Rate</div></div>
  </div>
  <div class="chart-section">
    <div>
      <h2>Test Results Distribution</h2>
      <div class="pie-chart">
        <svg viewBox="0 0 100 100">{pie_slices}</svg>
        <div class="center-text"><div class="big">{total}</div><div class="small">tests</div></div>
      </div>
    </div>
  ...
  <div class="table-section">
    ...
    <table>
      <thead>
        <tr><th style="width:40px">#</th><th>Test</th><th style="width:100px">Status</th><th style="width:80px">Duration</th><th style="width:30px"></th></tr>
      </thead>
...
def _generate_pie(total, passed, failed, skipped):
    ...
    slices = []
    data = [(c, color) for c, color in ((passed, "#3fb950"), (failed, "#f85149"), (skipped, "#d29922")) if c > 0]
    total_valid = sum(c for c, _ in data)
    if total_valid == 0:
        return '<circle cx="50" cy="50" r="40" fill="none" stroke="#30363d" stroke-width="8"/>'
    offset = 0
    for count, color in data:
        angle = count / total_valid * 360
        ...
        slices.append(f'<path d="M50,50 L{start_x:.2f},{start_y:.2f} A40,40 0 {large_arc},1 {end_x:.2f},{end_y:.2f} Z" fill="{color}"/>')
        offset += angle
    return "\n".join(slices)
...
def generate_report(test_results, env_info, output_path, title="API Automation Test Report"):
    ...
    for r in sorted_tests:
        pct = (r["duration"] / max_dur * 100) if max_dur else 0
        cls = "passed" if r["status"] == "passed" else "failed"
        bars.append(f'<div class="bar-row"><span class="bar-label">{r["name"][:60]}</span><div class="bar-track"><div class="bar-fill {cls}" style="width:{pct:.1f}%"></div></div><span class="bar-duration">{_duration_str(r["duration"])}</span></div>')

    icons = {"passed": "✅", "failed": "❌", "skipped": "⏭️", "error": "💥"}
    rows = []
    for i, r in enumerate(test_results, 1):
        s = r["status"]
        d = r.get("details", "").strip()
        rows.append(f'<tr><td style="color:#8b949e">{i}</td><td class="test-name">{r["name"]}</td><td><span class="status-badge {s}">{icons.get(s, "❓")} {s.title()}</span></td><td style="color:#8b949e">{_duration_str(r["duration"])}</td><td class="detail-toggle" style="cursor:pointer">{" " if d else ""}</td></tr>')
        if d:
            rows.append(f'<tr class="test-details"><td colspan="5"><div class="detail-content"><pre>{d}</pre></div></td></tr>')

    env_items = "".join(f'<div class="env-item"><span class="key">{k}</span><span class="value">{v}</span></div>' for k, v in env_info.items())
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    html = TEMPLATE.format(
        title=title, timestamp=timestamp, total_duration=_duration_str(total_duration),
        total=total, passed=passed, failed=failed, skipped=skipped, pass_rate=pass_rate,
        pie_slices=_generate_pie(total, passed, failed, skipped),
        duration_bars="".join(bars), test_rows="".join(rows), env_items=env_items,
    )
